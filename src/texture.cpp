...
namespace mini {
	uint32_t texture_t::get_width () const {
		return m_width;
	}

	uint32_t texture_t::get_height () const {
		return m_height;
	}

	uint32_t texture_t::get_mipmap_levels () const {
		return m_mipmap_levels;
	}

	GLenum texture_t::get_min_filter () const {
		return m_min_filter;
	}

	GLenum texture_t::get_mag_filter () const {
		return m_mag_filter;
	}

	GLenum texture_t::get_format () const {
		return m_format;
	}

	texture_t::texture_t (uint32_t width, uint32_t height, unsigned char * data) {
		m_width = width;
		m_height = height;
	...
	texture_t::texture_t (uint32_t width, uint32_t height, unsigned char * data, GLenum format, 
	...
	void texture_t::bind (GLenum slot) const {
		glActiveTexture (slot);
		glBindTexture (GL_TEXTURE_2D, m_texture);
	}

	void texture_t::m_initialize () {
	...
	std::shared_ptr<texture_t> texture_t::load_from_file (const std::string & file) {
		...
		return std::make_shared<texture_t> (width, height, data.data (), GL_RGBA);
	}
}